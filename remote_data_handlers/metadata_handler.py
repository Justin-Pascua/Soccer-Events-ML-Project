...
def get_competitions_df(verbose: bool = True):
    ...
    """
    competitions = {}
    with open(ROOT / 'competitions.json') as json_data:
...
def get_players_df(verbose: bool = True):
    ...
    """
    players = {}
    with open(ROOT / 'players.json') as json_data:
...
def get_teams_df(verbose: bool = True):
    ...
    """
    teams = {}
    with open(ROOT / 'teams.json') as json_data:
...
def get_eventids_df(verbose: bool = True):
    ...
    params:
        verbose: a bool indicating whether or not to print after the dataframe has been loaded.
    """
    eventids_df = pd.read_csv(ROOT / 'eventid2name.csv')
    if(verbose):
...
def get_tags_df(verbose: bool = True):
    ...
    """
    tags_df = pd.read_csv(ROOT / 'tags2name.csv')
    if(verbose):
...
def get_ranks_df(verbose: bool = True):
    ...
    params:
        verbose: a bool indicating whether or not to print after the dataframe has been loaded.
    """
    with open(ROOT / 'playerank.json') as json_data:
